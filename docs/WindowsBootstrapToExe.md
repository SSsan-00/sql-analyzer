...
- SQL 入力欄で `Ctrl+Space` による入力補助が開き、キーワードや別名候補を選べる
- `整形` ボタンまたは `Ctrl+Shift+F` で SQL が複数行へ整形される
- `CASE` の `WHEN / THEN / ELSE` が多段表示で整形される
- `CASE` の `WHEN` 条件に `AND / OR` がある場合も、条件ブロックが段落化される
- 構文エラーを含む SQL を解析または整形したとき、上部に `構文エラー` 一覧が出る
- 構文エラー一覧を選ぶと、SQL 側の該当箇所が赤背景で強調される
- 画面ヘッダーの `ワークスペース一覧` / `クエリ一覧` ボタンで一覧を展開・折りたたみできる
- ワークスペース一覧は `↑ / ↓` ボタンとドラッグ移動で並べ替えられる
- アプリ再起動後に最後のワークスペース状態が復元される
- アプリ再起動時、ワークスペース一覧とクエリ一覧は閉じた状態で開始される
...
- `列情報エクスポート` ボタンで、列確認用の `.txt` ファイルを保存できる
- 保存テキストでは、INSERT / UPDATE の対象列と値ブロックが `\\\\\\\\\\\` で分かれて表示される
