...
このドキュメントは、Postgres/T-SQL解析ツールのコードをどこから読むと全体像をつかみやすいかをまとめた開発者向けガイド。
...
5. `src/TSqlAnalyzer.Application/Analysis/PostgreSqlQueryAnalyzer.cs`
   - pgsqlparser-dotnet の PostgreSQL AST を独自モデルへ変換する実装を読む
6. `src/TSqlAnalyzer.Application/Presentation/QueryAnalysisTreeBuilder.cs`
   - 解析モデルを TreeView 向け表示ノードへ変換する流れを読む
7. `src/TSqlAnalyzer.WinForms/UI/AnalysisTreeViewBinder.cs`
   - 表示ノードを WinForms `TreeNode` へ落とし込む処理を読む
8. `src/TSqlAnalyzer.Application/Editor/ParseIssueTextSpanResolver.cs`
   - 構文エラーの行・列を SQL 文字範囲へ変換する処理を読む
9. `src/TSqlAnalyzer.Application/Editor/SqlInputAssistService.cs`
   - DB 非依存の入力補助候補を組み立てる処理を読む
10. `src/TSqlAnalyzer.Application/Workspace/WorkspaceStateManager.cs`
   - ワークスペース、クエリ一覧、展開状態の不変条件を読む
11. `src/TSqlAnalyzer.WinForms/MainForm.Workspace.cs`
...
- `Analysis`
  - ScriptDom / pgsqlparser-dotnet から独自モデルへ変換する
- `Presentation`
...
3. `FallbackSqlQueryAnalyzer.Analyze`
4. `ScriptDomQueryAnalyzer.Analyze` または `PostgreSqlQueryAnalyzer.Analyze`
5. `QueryAnalysisTreeBuilder.Build`
6. `AnalysisTreeViewBinder.Bind`
...
`SqlFormattingService` は T-SQL の `SELECT` 系を自前整形し、それ以外は ScriptDom の生成結果へフォールバックする。T-SQL として構文解析できない SQL は、`PostgreSqlFormattingService` で PostgreSQL として整形する。
...
2. `ScriptDomQueryAnalyzer.cs` または `PostgreSqlQueryAnalyzer.cs`
...
2. `PostgreSqlFormattingService.cs`
3. `tests/TSqlAnalyzer.Tests/Formatting/SqlFormattingServiceTests.cs`
...
- `tests/TSqlAnalyzer.Tests/Analysis/PostgreSqlQueryAnalyzerTests.cs`
  - PostgreSQL 解析と T-SQL 失敗時のフォールバックを確認する
...
- ScriptDom / pgsqlparser-dotnet の型を WinForms へ直接渡さない
