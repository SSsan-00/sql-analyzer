...
- ワークスペース一覧は `↑ / ↓` ボタンとドラッグ移動で並べ替えできる
- 画面ヘッダーの `ワークスペース一覧` / `クエリ一覧` ボタンで、それぞれの一覧を展開・折りたたみできる
- アプリ再起動後も、最後に開いていたワークスペースとクエリ本文を復元できる
- ワークスペース一覧とクエリ一覧の展開状態も再起動後に復元できる
...
`CASE` は次のように `WHEN` と `THEN` を分け、`ELSE` も別ブロックとして表示する。`WHEN` 条件に `AND / OR` があるときは条件ブロック自体も段落化する。
...
- `整形` ボタンまたは `Ctrl+Shift+F` で SQL が読みやすいレイアウトへ整形される
